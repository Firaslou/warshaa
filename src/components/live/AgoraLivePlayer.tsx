...
function uidFromString(value: string) {
  let hash = 2166136261;
  for (let i = 0; i < value.length; i++) hash = Math.imul(hash ^ value.charCodeAt(i), 16777619);
  return Math.abs(hash >>> 0) || 1;
}

export function AgoraLivePlayer({ liveEventId, channel, startupId, isHost, className }: Props) {
  const localVideoRef = useRef<HTMLDivElement>(null);
  const remoteVideoRef = useRef<HTMLDivElement>(null);
  const clientRef = useRef<any>(null);
  const tracksRef = useRef<any[]>([]);
  const uidRef = useRef<number>(uidFromString(`${liveEventId}:${isHost ? "host" : Math.random()}`));
  ...
  const getToken = useCallback(async () => {
    const { data, error } = await supabase.functions.invoke("agora-token", {
      body: { channel, uid: uidRef.current, role: isHost ? "host" : "audience" },
    });
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        if (!cancelled) {
          setStatus("failed");
          toast.error("Impossible de rejoindre le live Agora. Vérifiez la configuration Agora.");
        }
      }
    };
  ...
  useEffect(() => {
    const loadComments = async () => {
      const { data } = await supabase
        .from("live_comments")
        .select("id,user_id,user_name,content,like_count,created_at")
    ...
    const channelRef = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "live_comments", filter: `live_event_id=eq.${liveEventId}` }, ({ new: row }) => {
        setComments((prev) => [...prev, row as LiveComment]);
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "live_comments", filter: `live_event_id=eq.${liveEventId}` }, ({ new: row }) => {
        setComments((prev) => prev.map((comment) => comment.id === row.id ? row as LiveComment : comment));
      })
      .subscribe();
  ...
  const toggleCamera = async () => {
    const track = tracksRef.current[1];
    if (!track) return;
  ...
  const sendComment = async () => {
    ...
    const { error } = await supabase.from("live_comments").insert({
      ...
      user_name: userData.user.email?.split("@")[0] || "Visiteur",
      content,
    });
  ...
  const likeComment = async (comment: LiveComment) => {
    if (!isHost) return;
    await supabase.from("live_comments").update({ like_count: comment.like_count + 1 }).eq("id", comment.id);
  };

  return (
    <div className={cn("grid min-h-[520px] overflow-hidden rounded-3xl bg-zinc-950 text-white lg:grid-cols-[1fr_320px]", className)}>
      <div className="relative flex min-h-[420px] items-center justify-center bg-black">
        <div ref={isHost ? localVideoRef : remoteVideoRef} className="absolute inset-0 [&>video]:h-full [&>video]:w-full [&>video]:object-cover" />
      ...
      <aside className="flex min-h-[360px] flex-col border-l border-white/10 bg-zinc-900">
        <div className="border-b border-white/10 p-4"><div className="flex items-center gap-2 font-semibold"><Radio className="h-4 w-4 text-primary" /> Chat Warsha</div><p className="mt-1 text-xs text-white/50">Les commentaires sont en temps réel.</p></div>
        <div className="flex-1 space-y-2 overflow-y-auto p-3">
          {comments.map((comment) => <div key={comment.id} className="rounded-xl bg-white/5 p-2.5"><div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold text-primary">{comment.user_name}</span>{isHost && <button type="button" onClick={() => void likeComment(comment)} className="inline-flex items-center gap-1 text-[11px] text-white/60 hover:text-rose-400"><Heart className="h-3.5 w-3.5" />{comment.like_count || ""}</button>}</div><p className="mt-1 break-words text-xs text-white/90">{comment.content}</p></div>)}
          {comments.length === 0 && <p className="py-10 text-center text-xs text-white/40">Aucun commentaire pour le moment.</p>}
        </div>
