...
export function ExternalLiveEmbed({ url, platform: explicitPlatform }: { url: string; platform?: ExternalPlatform | null }) {
  ...
  const embedUrl = platform === "youtube" ? youtubeEmbed(url) : platform === "facebook" ? facebookEmbed(url) : null;
      ...
      <CardContent className="p-0">
        <div className="flex items-center justify-between border-b bg-card px-4 py-3">
          ...
          <Button asChild size="sm" variant="outline" className="gap-1">
            <a href={url} target="_blank" rel="noopener noreferrer">Ouvrir <ExternalLink className="h-3.5 w-3.5" /></a>
          </Button>
        </div>
        {embedUrl ? (
        ...
        {platform === "facebook" && embedUrl && (
          <div className="border-t bg-white p-3">
            ...
            <iframe src={facebookComments(url)} title="Commentaires Facebook" className="min-h-[280px] w-full border-0" loading="lazy" />
          </div>
        )}
      </CardContent>
    </Card>
