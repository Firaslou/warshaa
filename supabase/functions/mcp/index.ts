...
var get_creator_default = defineTool2({
  ...
  handler: async ({ slug }) => {
    ...
    const { data: products } = await client.from("products").select("id,name,description,price,image_url").eq("startup_id", startup.id).limit(50);
...
var search_products_default = defineTool3({
  ...
  handler: async ({ query, limit }) => {
    const client = sb3();
    const q = `%${query}%`;
    const { data, error } = await client.from("products").select("id,name,description,price,image_url,startup_id").or(`name.ilike.${q},description.ilike.${q}`).limit(limit ?? 20);
    if (error) return { content: [{ type: "text", text: error.message }], isError: true };
    return {
      content: [{ type: "text", text: JSON.stringify(data) }],
