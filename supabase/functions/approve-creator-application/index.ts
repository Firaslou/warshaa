...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (app.status === "approved") {
      return new Response(JSON.stringify({ error: "Already approved" }), { status: 409, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (userErr || !ownerRes?.user) {
      return new Response(JSON.stringify({ error: "Applicant user not found" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
      ...
      try {
        await admin.functions.invoke("send-transactional-email", {
          body: {
            ...
            templateData: { brandName: app.brand_name, startupSlug: startup!.slug },
          },
        });
      } catch (e) {
    ...
    return new Response(JSON.stringify({ success: true, startup_id: startup!.id, slug: startup!.slug }), {
